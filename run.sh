...
for method in cw  ; do # mcl ; do
    for N in 200 ; do # 20 50 100 200 500; do
        for n in 200 ; do # 5 10 20 50 100 200 500 ; do
            echo "$input-$method-e$e-N$N-n$n.csv" 
            java -Xms32G -Xmx250G -cp $JAVA_CLASSPATH  de.tudarmstadt.lt.wsi.WSI \
                -clustering $method \
                -N $N \
                -n $n \
                -e 0 \
